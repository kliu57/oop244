...
namespace sdds {
	...
	void AidMan::sort() {
		// bubble sort logic learned from IPC
		int i = 0;
		int k = 0;
		int diff1 = 0;
		int diff2 = 0;
		...
		//Orders the date by total revenue in decending order (highest-lowest)
		for (i = m_iProductNum - 1; i > 0; i--) {
			for (k = 0; k < i; k++) {
				// sort by diff btw qtyNeeded and qty
				diff1 = m_iProducts[k]->qtyNeeded() - m_iProducts[k]->qty();
				diff2 = m_iProducts[k+1]->qtyNeeded() - m_iProducts[k+1]->qty();

				if (diff1 < diff2) {
					//Creating temp values
					tempProduct = m_iProducts[k];

					//Moving the lower one to the next index positon
					m_iProducts[k] = m_iProducts[k+1];

					//Moving the higher one to the current index position
					m_iProducts[k+1] = tempProduct;
				}
			}
		}
		cout << "Sort completed!\n";
	...
	void AidMan::shipItems() {
		...
		ofs.open("shippingOrder.txt");	// open file for writing

		ofs << "Shipping Order, Date: " << Date() << "\n";	// current dat

		ofs << " ROW |  SKU  | Description                         | Have | Need |  Price  | Expiry\n";
		ofs << "-----+-------+-------------------------------------+------+------+---------+-----------\n";
