...
namespace sdds {
    ...
    void read(std::ifstream* fptr, char* cString, unsigned int maxSize, char delimiter) {
        ...
        // skipping leading white space chars
        do {   
            (*fptr).get(ch); // reads one character from input
        } while (isSpace(ch));
        // read char by char util hitting delimiter or maxSize OR END OF FILE REACHED
        for (i = 0; i < maxSize && ch != delimiter && !(*fptr).eof();i++) {
            cString[i] = ch;
            // read the cString stopping at the size limit
            if (i < maxSize-1) (*fptr).get(ch); 
        }       
        cString[i] = 0; // make sure the cString is null terminated
    }

    int readPositiveInt(std::ifstream* fptr, int minSize, int maxSize) {   // returns -1 if failed
        int intValue = -1;
        int tempValue = -1;

        (*fptr) >> tempValue; // reads integer from input
